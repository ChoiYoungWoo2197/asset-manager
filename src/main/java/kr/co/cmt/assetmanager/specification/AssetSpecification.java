...
public class AssetSpecification {
    public static Specification<Asset> searchWith(Map<String, Object> searchKeyword) {
        return (Specification<Asset>) ((root, query, builder) -> {
            List<Predicate> predicate = getPredicateWithKeyword(searchKeyword, root, builder);
            return builder.and(predicate.toArray(new Predicate[0]));
        });
    }

    private static List<Predicate> getPredicateWithKeyword(Map<String, Object> searchKeyword, Root<Asset> root, CriteriaBuilder builder) {
